...
public class Threads18 {
	...
	public static void addThreadsToPool() {
		 ...
		 * You can also java code execute repetitively based on a time period
		...
		eventPool.scheduleAtFixedRate(new CheckSystemTime18(), 0, 2, SECONDS);
		eventPool.scheduleAtFixedRate(new PerformSystemCheck18("Mail"), 5, 5, SECONDS);
		eventPool.scheduleAtFixedRate(new PerformSystemCheck18("Calendar"), 10, 10, SECONDS);
		
		//Thread.activeCount returns the number of threads running
		System.out.println("Number of threads: " + Thread.activeCount());
		
		//Why does it says there are 4 threads when we expected 3?
		//4 because main thread is running
		...
		// that created the new thread. Top priority is 10, lowest priority is 1 (main = 5)
